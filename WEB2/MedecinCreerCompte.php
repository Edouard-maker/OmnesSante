...
<head>
    ...
    if (isset($_POST['btnsignup'])) {
        ...
        // Insert records
        if ($isValid) {
            $insertSQL = "INSERT INTO medecin(prenomPro,nomPro,courrielPro,idPro,mdpPro) values(?,?,?,?,?)";
            $stmt = $con->prepare($insertSQL);
            $stmt->bind_param("sssss", $fname, $lname, $email, $login, $password);
            $stmt->execute();
            $stmt->close();

            $success_message = "Bravo ! Compte médecin créé";
        }
    }
    ?>
</head>
        ...
        <div class='row'>
            ...
            <div class='col-md-6'>
                <form method='post' action=''>
                    ...
                    <p> Vous avez déjà un compte ? <a href="MedecinLogin.php"> Connectez-vous ici </a></p>
                </form>
            </div>
