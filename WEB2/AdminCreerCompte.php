...
<head>
    ...
    if (isset($_POST['btnsignup'])) {
        ...
        if ($fname == '' || $lname == '' || $email == '' || $login == '' || $password == '' || $confirmpassword == '') {
            $isValid = false;
        ...
        if ($isValid) {
            ...
            $stmt = $con->prepare("SELECT * FROM administrateur WHERE courrielAdmin = ?");
        ...
        // Insert records
        if ($isValid) {
            $insertSQL = "INSERT INTO administrateur(prenomAdmin,nomAdmin,courrielAdmin,loginAdmin,mdpAdmin) values(?,?,?,?,?)";
            $stmt = $con->prepare($insertSQL);
            $stmt->bind_param("sssss", $fname, $lname, $email, $login, $password);
            $stmt->execute();
            $stmt->close();

            $success_message = "Bravo ! Compte admin créé";
        }
    }
    ?>
</head>
    ...
    <div class='container'>
        <div class='col-md-4'>
            <form method='post' action=''>
                ...
                <p> Vous avez déjà un compte ? <a href="AdminLogin.php"> Connectez-vous ici </a></p>
            </form>
        </div>
    </div>
